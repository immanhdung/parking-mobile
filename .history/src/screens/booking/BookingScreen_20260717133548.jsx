...
export default function BookingScreen({ navigation, route }) {
  ...
  const canNext = () => {
    ...
    if (step === 1) return !!form.vehicleInfo.licensePlate.trim() && !!form.scheduledDate && !isTimeInPast(form.scheduledDate, form.startTime)
    return true
  }
  ...
  // Only "giờ vào" (check-in time) on today's date can be in the past — future dates have no such restriction.
      ...
      DateTimePickerAndroid.open({
        ...
        onChange: (event, time) => {
          if (event.type === 'set' && time) {
            ...
            if (type === 'start' && isTimeInPast(form.scheduledDate, timeStr)) {
